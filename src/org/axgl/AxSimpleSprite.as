package org.axgl {
	...
	/**
	 * An <code>AxSimpleSprite</code> is the entity that makes up most game objects. You can load an image, rotate it, change
	 * the color, move it, and more. Game objects that will be visible on screen should extend this class.
	 */
	public class AxSimpleSprite extends AxModel {
		 ...
		 */
		public function AxSimpleSprite(x:Number = 0, y:Number = 0, graphic:Class = null, frameWidth:uint = 0, frameHeight:uint = 0) {
			super(x, y, VERTEX_SHADER, FRAGMENT_SHADER, 4, "AxSimpleSprite");
		 ...
		 */
		public function load(graphic:*, frameWidth:uint = 0, frameHeight:uint = 0, softLoad:Boolean = false):AxSimpleSprite {
			if (softLoad) {
		 ...
		 * @return The sprite instance.
		 */
		public function create(width:uint, height:uint, color:uint):AxSimpleSprite {
		 ...
		 */
		public function bounds(width:uint, height:uint, offsetX:int, offsetY:int):AxSimpleSprite {
			this.width = width;
			this.height = height;
		 ...
		 * @return The sprite instance.
		 */
		public function addAnimation(name:String, frames:Array, framerate:uint = 15, looped:Boolean = true, callback:Function = null):AxSimpleSprite {
		 ...
		 * @return The sprite instance.
		 */
		public function animate(name:String, reset:Boolean = false):AxSimpleSprite {
		 ...
		 * @return The sprite instance.
		 */
		public function show(frame:uint):AxSimpleSprite {
			this.animation = null;
			this.frame = frame;
			calculateFrame();
			return this;
		...
		public function addEffect(effect:AxSpriteEffect):AxSimpleSprite {
			/*if (effects == null) {
		...
		public function clearEffects():AxSimpleSprite {
			for each(var effect:AxSpriteEffect in effects) {
				if (effect.active) {
					effect.destroy();
				}
		...
		public function startFlicker(duration:Number = 0, callback:Function = null, rate:uint = 1, type:uint = AxFlickerSpriteEffect.BLINK):AxSimpleSprite {
		...
		public function stopFlicker():AxSimpleSprite {
		...
		public function fadeOut(duration:Number = 1, targetAlpha:Number = 0, callback:Function = null):AxSimpleSprite {
		...
		public function fadeIn(duration:Number = 1, targetAlpha:Number = 1, callback:Function = null):AxSimpleSprite {
			if (fadeEffect != null && fadeEffect.active) {
				fadeEffect.destroy();
			}
			addEffect(fadeEffect = new AxAlphaSpriteEffect(duration, callback, targetAlpha));
		...
		public function grow(duration:Number = 1, targetXScale:Number = 2, targetYScale:Number = 2, callback:Function = null):AxSimpleSprite {
		...
		override public function overlaps(other:AxRect):Boolean {
			...
			if (other is AxGroup) {
				...
				/*var sprites:Vector.<AxSimpleSprite> = (other as AxCloud).members;
				for each (var s:AxSimpleSprite in sprites) {
		...
		/**
		 * A static sprite index buffer that all AxSimpleSprites will use.
		 */
