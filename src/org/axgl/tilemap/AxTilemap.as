				...
				for (var y:uint = sy; y <= ey; y++) {
					...
					if (!collide) {
						...
						if (tile.collision == ANY) {
							...
							if (tile.collision & RIGHT /*&& ow > tw*/ && (!tile.oneWay || target.previous.x + AxU.EPSILON >= tw)) {
								if (ow >= opw) {
									tile.previous.x = tile.x = tw;
								}
								overlapped = callback(target, tile) || overlapped;
								tile.previous.x = tile.x = tx;
							} else if (tile.collision & LEFT /*&& target.x < tx*/ && (!tile.oneWay || opw - AxU.EPSILON <= tx)) {
								if (ow <= opw) {
							...
							if (tile.collision & DOWN /*&& oh > th*/ && (!tile.oneWay || target.previous.y + AxU.EPSILON >= th)) {
								if (oh >= oph) {
									tile.previous.y = tile.y = th;
								}
								overlapped = callback(target, tile) || overlapped;
								tile.previous.y = tile.y = ty;
							} else if (tile.collision & UP /*&& target.y < ty*/ && (!tile.oneWay || oph - AxU.EPSILON <= ty)) {
